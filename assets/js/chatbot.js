...
(() => {
  ...
  const onOutsideClick = (e) => {
    ...
    const clickedInside =
      target.closest("[data-tf-chatbot-panel]") ||
      target.closest("[data-tf-chatbot-launch]");
  ...
  const send = async () => {
    ...
    const lastUser = messagesEl.querySelector(
      ".tf-chatbot-row--user .tf-chatbot-bubble",
    );
    ...
    await typeAssistantText(
      String(assistantText || "").trim() || "No response yet.",
    );
